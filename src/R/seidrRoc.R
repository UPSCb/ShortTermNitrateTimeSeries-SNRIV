...
#' ```
res <- plotRoc(here("data/seidr/results/ResRoc/aggregated_roc_WithNegative.tsv"))
...
#' ```
files <- dir(here("data/seidr/results/ResRoc/"),pattern="backbone.*WithNegative\\.tsv",full.names=TRUE)
names(files) <- gsub("backbone-|_roc_WithNegative.tsv","",basename(files))
...
files <- dir(here("data/seidr/results/ResRoc/"),pattern="backbone.*NoNegative\\.tsv",full.names=TRUE)
names(files) <- gsub("backbone-|_roc_NoNegative.tsv","",basename(files))
files <- files[order(as.integer(sub("-percent","",names(files))))]
resb <- lapply(files,plotRoc)
